...
if(NOT CPACK_PACKAGE_VENDOR OR NOT CPACK_PACKAGE_DESCRIPTION_SUMMARY OR
   NOT VERSION OR NOT GIT_REVISION)
  message(FATAL_ERROR "Need CPACK_PACKAGE_VENDOR, CPACK_PACKAGE_DESCRIPTION_SUMMARY, "
                      "VERSION and GIT_REVISION for GNUModule")
endif()
...
# Snapshot module
################################################################################
if(MODULE_SNAPSHOT_DIR) # comes from Buildyard
  execute_process(COMMAND date "+%Y-%m-%d" OUTPUT_VARIABLE MODULE_DATE
                  OUTPUT_STRIP_TRAILING_WHITESPACE)
  execute_process(COMMAND date "+%Y-%m-%d" "-d now + 6 months" OUTPUT_VARIABLE MODULE_EXPIRATION_DATE
                  OUTPUT_STRIP_TRAILING_WHITESPACE)
  ...
  add_custom_target(snapshot
    COMMAND ${CMAKE_COMMAND} -DMODULE_PACKAGE_NAME=${MODULE_PACKAGE_NAME}
                             -DMODULE_VERSION=${MODULE_VERSION}
                             -DMODULE_FILENAME=${MODULE_FILENAME}
                             -DMODULE_MESSAGE_AFTER_LOAD=${MODULE_MESSAGE_AFTER_LOAD}
                             -P WriteModuleFile.cmake
    COMMAND ${CMAKE_COMMAND} -E copy ${MODULE_FILENAME} ${MODULE_MODULEFILES}/${MODULE_FILENAME}
    COMMAND ${CMAKE_COMMAND} -E create_symlink ${MODULE_MODULEFILES}/${MODULE_FILENAME}
                                               ${MODULE_MODULEFILES}/${MODULE_PACKAGE_NAME}/latest
    WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
    COMMENT "Creating snapshot module ${MODULE_FILENAME} at ${MODULE_MODULEFILES}" VERBATIM
    DEPENDS snapshot_install)
endif()
